...
namespace SegurosCrecerAPI.Controllers
{
    ...
    public class TipoRamoController : Controller
    {
        private readonly SegurosCrecerContext _context;

        public TipoRamoController(
        ...
        {
            return Ok(await _context.TipoRamos.ToListAsync());
        }

        [HttpGet("loadSelect")]
        public async Task<IActionResult> LoadSelect()
        {
            var result = await _context.TipoRamos.ToListAsync();
